...
function Characteristic(options) {
  ...
  if (options.onPrepareWriteRequest) {
    this.onPrepareWriteRequest = options.onPrepareWriteRequest;
  }

  if (options.onExecuteWriteRequest) {
    this.onExecuteWriteRequest = options.onExecuteWriteRequest;
  }
  ...
  this.on('prepareWriteRequest', this.onPrepareWriteRequest.bind(this));
  this.on('executeWriteRequest', this.onExecuteWriteRequest.bind(this));
...
Characteristic.prototype.onPrepareWriteRequest = function(data, offset, callback) {
  callback(this.RESULT_UNLIKELY_ERROR);
};

Characteristic.prototype.onExecuteWriteRequest = function(flag, callback) {
  callback(this.RESULT_UNLIKELY_ERROR);
};
